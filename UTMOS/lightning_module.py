...
import os
import numpy as np
from UTMOS.model import load_ssl_model, PhonemeEncoder, DomainEmbedding, LDConditioner, Projection
import pathlib
...
class BaselineLightningModule(pl.LightningModule):
    ...
    def construct_model(self):
        cp_model_path = pathlib.Path(__file__).parent.resolve() / "wav2vec_small.pt"
        # windows path
        if isinstance(cp_model_path, pathlib.WindowsPath):
            splitted = list(cp_model_path.parts)
            splitted[0] = splitted[0][:2]
            cp_model_path = '/'.join(splitted)
        else:
            cp_model_path = str(cp_model_path)
        self.feature_extractors = nn.ModuleList([
            load_ssl_model(cp_path=cp_model_path),
